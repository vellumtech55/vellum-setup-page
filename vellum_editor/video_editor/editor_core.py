...
def compute_rms_times(y, sr):
    """RMS energy + frame timestamps, computed once and shared across
    detect_sentences/score_sentences instead of each recomputing it."""
    rms   = librosa.feature.rms(y=y)[0]
    times = librosa.frames_to_time(range(len(rms)), sr=sr)
    return rms, times


def detect_sentences(y, sr, rms=None, times=None):
    """Group continuous speech into (start, end) segments."""
    if rms is None or times is None:
        rms, times = compute_rms_times(y, sr)
...
def score_sentences(y, sr, sentences, rms=None, times=None):
    if rms is None or times is None:
        rms, times = compute_rms_times(y, sr)

    scored = []
    for start, end in sentences:
        # searchsorted turns each lookup into O(log n) instead of scanning
        # every frame in the track for every sentence
        lo = np.searchsorted(times, start, side="left")
        hi = np.searchsorted(times, end, side="right")
        energies   = rms[lo:hi]
        avg_vol    = float(np.mean(energies)) if len(energies) else 0.0
        duration   = end - start
        scored.append({
            "start":      start,
            "end":        end,
...
def process_video(
    ...
    rms, times  = compute_rms_times(y, sr)
    sentences   = detect_sentences(y, sr, rms=rms, times=times)
    scored      = score_sentences(y, sr, sentences, rms=rms, times=times)
